# coding: utf-8

"""
    Liara Mail API Documentaion

    A fully featured mail delivery platform for incoming & outgoing e-mail

    The version of the OpenAPI document: 1.0.0
    Contact: support@liara.ir
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest
import datetime

from openapi_client.models.mail_server_data import MailServerData  # noqa: E501

class TestMailServerData(unittest.TestCase):
    """MailServerData unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> MailServerData:
        """Test MailServerData
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `MailServerData`
        """
        model = MailServerData()  # noqa: E501
        if include_optional:
            return MailServerData(
                mail_server = openapi_client.models.mail_server_response.MailServerResponse(
                    dns_setup = openapi_client.models.check_dns_response_dns_setup.CheckDNSResponse_dns_setup(
                        dkim = openapi_client.models.check_dns_response_dns_setup_dkim.CheckDNSResponse_dns_setup_dkim(
                            dkim_error = '', 
                            dkim_record = '', 
                            dkim_record_name = '', 
                            dkim_status = '', ), 
                        mx_record = openapi_client.models.check_dns_response_dns_setup_mx_record.CheckDNSResponse_dns_setup_mx_record(
                            mx_error = '', 
                            mx_status = '', 
                            mx_record_value = '', ), 
                        return_path = openapi_client.models.check_dns_response_dns_setup_return_path.CheckDNSResponse_dns_setup_return_path(
                            return_path_domain = '', 
                            return_path_error = '', 
                            return_path_status = '', 
                            return_path_value = '', ), 
                        spf = openapi_client.models.check_dns_response_dns_setup_spf.CheckDNSResponse_dns_setup_spf(
                            spf_error = '', 
                            spf_record = '', 
                            spf_status = '', ), ), 
                    domain = '', 
                    records_status = '', 
                    status = '', 
                    rate_limit_tier = openapi_client.models.mail_server_response_rate_limit_tier.MailServerResponse_rateLimitTier(
                        hourly = 1.337, 
                        daily = 1.337, 
                        monthly = 1.337, ), 
                    created_at = '', 
                    id = '', 
                    smtp_server = '', 
                    smtp_port = 1.337, 
                    root_part_of_domain = '', )
            )
        else:
            return MailServerData(
        )
        """

    def testMailServerData(self):
        """Test MailServerData"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
